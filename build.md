...
第二项验证 owner token、generation、跨 thread 单调 claim、运行态 owner 门禁、心跳、TTL 与同 thread 独占。第三项验证首次持久化只发布一个共享 load Promise、全部写操作串行化、director 旧签名兼容、user append/mode/workspace 保存失败回滚，以及删除缺少 guard 或在线性化前失权时失败关闭并保留原线程。
...
- user append/mode/workspace 保存失败必须条件回滚本次内存修改；后续成功快照不得夹带失败值。`session.run()` 抛错或未进入 running 必须回滚用户消息且不能标记已启动。
...
- 实现提交：`228c00b4aec7702748711de91d3927cc46656aa4`，tree `864c11a6e5faac2afc4621de21f4ab16cc9c7973`。
- 无重试执行 `npm ci`、侧栏构建、13/13 Node 自测文件、branding 和 `git diff --check`，全部通过；bundle 为 `220.9kb`，thread reservation 为 `60/60`，ConversationStore 为 `48/48`，multi-window routing 合同为 `PASS`，branding 为 22 文件。
