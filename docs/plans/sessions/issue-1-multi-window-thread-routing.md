...
- 决策：保留 `AgentSession` 的稳定 owner、心跳、TTL、同 thread 独占和多 thread 并行；删除 `AgentPanel` 对其它窗口运行 thread 的自动 `openThread()` 接管，提示条点击改为 `newChat()`，并为同 owner 重挂载增加 generation 与 owner+generation 级单调 claim fence。运行中正常卸载保留 owner 锚点，尚未绑定的临时 claim 则以显式 abandon 语义清除锚点。
...
change_contract:
  ...
  stale_verdict_invalidation_refs:
    ...
    - 2026-08-14 源码 checkpoint 门禁只属于 git:ce6bf30c585dede0ce76c56ba8d1bab6f0b0fe29；治理提交后必须对 final exact HEAD/tree 重新审查
  regression_checks:
    ...
    - surface: reservation-and-heartbeat
      ...
      expected_result: generation fence、owner+generation 最高 claim、运行态 owner 门禁、正常运行中 release 锚点、临时 claim abandon、失败尝试发布水位、附属目标、旧 reservation 清理、心跳与 TTL 全部通过
  ...
  protected_feature_replay:
    ...
    known_good_features:
      - feature: thread reservation and current-thread recovery
        ...
        post_change_replay_ref: git:ce6bf30c585dede0ce76c56ba8d1bab6f0b0fe29-local-source-gate:2026-08-14T09:50:04+08:00
        expected_result: reservation suite and aggregate selftests pass
        actual_result: PowerShell 完整链无重试通过 sidebar bundle 221.5kb、13/13 Node 自测文件和 branding 22 文件检查；reservation 70/70、ConversationStore 60/60、正常运行中 release 后同 owner 重挂载、临时 claim abandon、历史读取和非当前删除 idle-to-running 门禁、任务结束 TTL 回收、metadata 保存失败回滚、director 旧签名、共享冷启动、串行 mutation、配置 intent 与生产删除 helper 动态合同通过
...
execution_evidence:
  test:
    ...
    result_ref: git:ce6bf30c585dede0ce76c56ba8d1bab6f0b0fe29-local-source-gate:2026-08-14T09:50:04+08:00-sidebar-and-13-selftests-pass
  build:
    command_ref: build.md#侧栏构建
    result_ref: git:ce6bf30c585dede0ce76c56ba8d1bab6f0b0fe29-local-source-gate:2026-08-14T09:50:04+08:00-bundle-221.5kb
  review:
    command_ref: build.md#独立审查
    result_ref: git:ac55b198a4222a55b2c5a45f6d9fa84dfd42e62e-request-changes; git:e84e4b18ac250614979876b68e810a6fc2845d16-request-changes-p1-1-p2-2; git:bc639e34389331ebc950989e4b431160992b7f06-request-changes-p1-3-p2-1-reviewer-019ffd25-55db-7473-aab5-a42ac5c9b963; git:98e080540b823327f3e52ceed2b821784d3fc7b6-request-changes-p1-2-p2-1-reviewer-019ffd82-1326-7e42-b2c0-2355aecf81ad; git:78464614385e6599dbdbafe3945938c5d2341c54-request-changes-p1-2-p2-1-reviewer-d5261c5b-1250-4855-99e4-e29f1185d0c0; git:ee665652da982dc40c8dd8b71a692d48cb5aa8fd-request-changes-p1-2-p2-1-reviewer-7a7dc2f6-5372-444b-b9d2-0945acf807c5; reviewer:51befd7e-f709-449c-b919-72614292f4be-upstream-pr-resolution-gate-failure-no-source-review; git:f92d93752aecbec3edf33bce5486fe8d95935371-provider-503-no-verdict; reviewers:019ffcda-2437-7791-a95a-54cab6ca68a8,019ffcee-5f0d-7892-977e-eced1beb2c71-shutdown-no-verdict; fresh final exact-head rereview pending
  verification:
    command_ref: build.md#交付边界检查
    result_ref: git:ce6bf30c585dede0ce76c56ba8d1bab6f0b0fe29-local-source-gate:2026-08-14T09:50:04+08:00-lockfile-ignored-bundle-and-12-file-boundary-pass
  closeout:
